package com.godrej.surveys.onboarding.sqlprovider;

public class OnboardingSurveySQLProvider {

private StringBuilder baseQuery = new StringBuilder();
	
	public OnboardingSurveySQLProvider() {//'givs_touchpoint' as app_id,
		...
		.append(",'ECRM' AS tag_field3, '6063132' as tag_field18 ")
	...
	public final String getContactSQL() {
		...
		query.append(baseQuery.toString())
			.append(getBasicWhereClause())
				 .append(" AND welcome_letter_sent_date__c = (date(to_char(now(),'mm-dd-yyyy'))- integer'3') ")
				 .append(" AND a.Customer_OnBoarding_Survey_Sent_Date__c is null AND a.Customer_onboarding_survey_sent__c <> true ")
	...
	public final String getFirstReminderSQL() {
		...
		.append(" ELSE 'POST_POSSESSION' END AS surveyType")
		.append(",propstrength__property_name__c AS propertyName")
		.append(",'ECRM' AS field3, '6063132' as field18 ")
	...
	public final String getSecondReminderSQL() {
		...
		.append( " WHERE A.propstrength__project__c=#{sfid} ")
				 ...
				 * )
				 */
		.append(" AND  Customer_OnBoarding_Survey_Sent_Date__c =date(to_char(now(),'mm-dd-yyyy'))- integer'4' ")
		.append(" AND CustomerOnBoard_SecondReminder_Sent_Date__c IS NULL ")
		.append(" ORDER BY a.propstrength__booking_date__c,a.propstrength__primary_applicant_name__c ASC  ");
		return query.toString();	
	}
	...
	public String getStatusUpdateQuery() {
		...
		query.append("UPDATE salesforce.propstrength__application_booking__c a SET ") 
			.append(" Customer_OnBoarding_Survey_Sent_Date__c= now(),Customer_onboarding_survey_sent__c=true")
			.append(" WHERE ") 
	...
	public String getStatusUpdateFromContactLogsQuery() {
		StringBuilder query =  new StringBuilder();
		query.append("UPDATE salesforce.propstrength__application_booking__c a SET ") 
			.append(" Customer_OnBoarding_Survey_Sent_Date__c = survey_sent_date")
			.append(",Customer_onboarding_survey_sent__c= survey_sent")
	...
	public String getClearingUpdateQuery() {
		...
		query.append("UPDATE salesforce.propstrength__application_booking__c SET ") 
			.append("Customer_OnBoarding_Survey_Sent_Date__c = null WHERE sfid in ( SELECT sfid FROM (") 
				...
				.append(" AND a.propstrength__applicant_email__c is not null ")
				 .append(" AND welcome_letter_sent_date__c = (date(to_char(now(),'mm-dd-yyyy'))- integer'3') ")
				.append(" AND a.Customer_OnBoarding_Survey_Sent_Date__c = CURRENT_DATE")
				.append( " ORDER BY ") 
				.append(" a.propstrength__booking_date__c, ")  
	...
	public String getUpdateFirstReminderStatusSQL() {
		...
		.append(" SET Customer_OnBoard_Last_Reminder_Sent_Date__c=now()")
	...
	public String getUpdateSecondReminderStatusSQL() {
		StringBuilder query = new StringBuilder();
		query.append("UPDATE salesforce.propstrength__application_booking__c")
		.append(" SET CustomerOnBoard_SecondReminder_Sent_Date__c=now()")
		.append(" WHERE sfid=#{sfid}");	
		return query.toString();
	}
}
