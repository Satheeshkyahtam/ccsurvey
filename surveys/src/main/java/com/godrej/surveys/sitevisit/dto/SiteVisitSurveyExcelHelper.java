...
public class SiteVisitSurveyExcelHelper {

	public static ByteArrayInputStream tutorialsToExcel(List<SiteVisitSurveyContactDto> contacts, String instanceId) throws Exception {
		
		List<String[]> csvData = createCsvDataForSurvey(contacts);

        // default all fields are enclosed in double quotes
        // default separator is a comma
        try (CSVWriter writer = new CSVWriter(new FileWriter("D:\\Satheesh\\Projects\\Litmus World\\SiteVisit\\"+instanceId+".csv"))) {
            writer.writeAll(csvData);
        }
		return null;
		
	}
	private static List<String[]> createCsvDataForSurvey(List<SiteVisitSurveyContactDto> contacts) {
        ...
        Iterator<SiteVisitSurveyContactDto> it = contacts.iterator();
		while (it.hasNext()) {
			SiteVisitSurveyContactDto contact = it.next();
			list.add(new String[] { "Touchpoint", "mxfz_touchpoint", contact.getUser_phone(), contact.getTransactionDate(),contact.getUser_email(),contact.getFirstName(),contact.getName(),contact.getBookingDate(),contact.getSentDate(),"FALSE"
