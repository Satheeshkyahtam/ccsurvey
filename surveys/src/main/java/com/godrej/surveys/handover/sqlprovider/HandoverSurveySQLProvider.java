package com.godrej.surveys.handover.sqlprovider;

public class HandoverSurveySQLProvider {

private StringBuilder baseQuery = new StringBuilder();
	
	public HandoverSurveySQLProvider() {
		...
		.append(",'ECRM' AS tag_field3, '6119246' as tag_field18 ")
	...
	public final String getContactSQL() {
		...
		.append(getBasicWhereClause())
				.append(" AND a.handover_date__c >= (date(to_char(now(),'mm-dd-yyyy'))- integer'7') AND handover_date__c <= (date(to_char(now(),'mm-dd-yyyy')))")
				.append(" AND a.Customer_Hand_Over_Survey_Sent_Date__c is null AND a.Handover_Survey_Sent__c <> true ")
	...
	public final String getFirstReminderSQLFromStaging() {
		StringBuilder query =  new StringBuilder();
		query.append(" Select  email,field20 ,mobile ,firstName,lastName ,bookingDate ,") 
		.append("	name,segmentCode ,transactionDate , field1 ,field15 ,field2 ,field4 ,field6 ,")
		.append("	field8 ,field9 ,field11 ,field13 ,field14 ,field16 ,surveyType ,propertyName ,") 
	...
	public final String getFirstReminderSQL() {
		StringBuilder query = new StringBuilder();
		query.append("SELECT a.propstrength__applicant_email__c as email,a.rm_name__c field20 ")
		...
		.append(" ELSE 'POST_POSSESSION' END AS surveyType")
		.append(",propstrength__property_name__c AS propertyName")
		.append(",'ECRM' AS field3, '6119246' as field18 ")
	    ...
	    .append( " WHERE A.propstrength__project__c=#{sfid} ")
				 ...
				 * )
				 */
		.append(" AND Customer_Hand_Over_Survey_Sent_Date__c =date(to_char(now(),'mm-dd-yyyy'))- integer'2' ")
		.append(" AND a.Hand_Over_Last_Reminder_Sent_Date__c IS NULL ")
	...
	public final String getSecondReminderSQL() {
	    ...
	    .append( " WHERE A.propstrength__project__c=#{sfid} ")
				 ...
				 * )
				 */
		.append(" AND  Customer_Hand_Over_Survey_Sent_Date__c =date(to_char(now(),'mm-dd-yyyy'))- integer'4' ")
		.append(" AND HandOver_Second_Reminder_Sent_Date__c IS NULL ")
		.append(" ORDER BY a.propstrength__booking_date__c,a.propstrength__primary_applicant_name__c ASC  ");
		return query.toString();	
	}
	...
	public String getStatusUpdateQuery() {
		...
		query.append("UPDATE salesforce.propstrength__application_booking__c a SET ") 
			.append(" Customer_Hand_Over_Survey_Sent_Date__c= now(),Handover_Survey_Sent__c=true")
			.append(" WHERE ") 
	...
	public String getStatusUpdateFromContactLogsQuery() {
		StringBuilder query =  new StringBuilder();
		query.append("UPDATE salesforce.propstrength__application_booking__c a SET ") 
			.append(" Customer_Hand_Over_Survey_Sent_Date__c = survey_sent_date")
			.append(",Handover_Survey_Sent__c= survey_sent")
	...
	public String getClearingUpdateQuery() {
		...
		query.append("UPDATE salesforce.propstrength__application_booking__c SET ") 
			.append("Customer_Hand_Over_Survey_Sent_Date__c = null WHERE sfid in ( SELECT sfid FROM (") 
				...
				.append(" AND a.propstrength__applicant_email__c is not null ")
				.append(" AND a.handover_date__c >= (date(to_char(now(),'mm-dd-yyyy'))- integer'7') AND handover_date__c <= (date(to_char(now(),'mm-dd-yyyy')))")
				.append(" AND a.Customer_Hand_Over_Survey_Sent_Date__c = CURRENT_DATE")
				.append( " ORDER BY ") 
				.append(" a.propstrength__booking_date__c, ")  
	...
	public String getUpdateFirstReminderStatusSQL() {
		...
		.append(" SET Hand_Over_Last_Reminder_Sent_Date__c=now()")
	...
	public String getUpdateSecondReminderStatusSQL() {
		StringBuilder query = new StringBuilder();
		query.append("UPDATE salesforce.propstrength__application_booking__c")
		.append(" SET HandOver_Second_Reminder_Sent_Date__c=now()")
		.append(" WHERE sfid=#{sfid}");	
		return query.toString();
	}
}
